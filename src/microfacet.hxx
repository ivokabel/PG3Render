...
namespace Microfacet
{
                ...
                Vec3f &oReflectDir)
    {
        ...
        const float tmp1 =
                (aRoughnessAlpha * std::sqrt(aUniSamples.x))
            / std::sqrt(1.0f - aUniSamples.x);
        const float thetaM  = std::atan(tmp1);
        const float phiM    = 2 * Math::kPiF * aUniSamples.y;
        ...
        const Vec2f &aUniSamples)
    {
        if (aThetaI < 0.0001f)
        {
            ...
            const float sampleXClamped  = std::min(aUniSamples.x, 0.9999f);
            const float radius          = Math::SafeSqrt(sampleXClamped / (1 - sampleXClamped));
            const float phi             = 2 * Math::kPiF * aUniSamples.y;
        ...
        else
        {
            ...
            float A = 2.0f * aUniSamples.x / G1 - 1.0f; // TODO: dividing by G1, which can be zero?!?
            if (std::abs(A) == 1.0f)
            ...
            float ySign;
            float yHalfSample;
            if (aUniSamples.y > 0.5f) // pick one positive/negative interval
            {
                ySign       = 1.0f;
                yHalfSample = 2.0f * (aUniSamples.y - 0.5f);
            }
            else
            {
                ySign       = -1.0f;
                yHalfSample = 2.0f * (0.5f - aUniSamples.y);
            }
            const float z =
        ...
        const Vec2f &aUniSamples)
    {
        ...
        Vec2f slopeStretch;
        SampleGgxP11(slopeStretch, thetaWolStretch, aUniSamples);
